...
def upload_list(cursor):
    ...
    if len(list_for_upload_ozon) > 0:
        logger.info("Список продуктов к загрузке сформирован...", 1)
    else:
        logger.error("Список продуктов к загрузке не сформирован...", 1)
    return list_for_upload_ozon


def change_keys(i):
    name = i
# АНАЛОГИЧНО БАЗОВЫМ КАРТОЧКАМ ВЗЯТЬ INSERT КАРТОЧКИ ИЗ ТАБЛИЦЫ OZON_BASE_CARD В БД
...
# ПО ТЕКУЩЕМУ НАЗВАНИЮ PRODUCT_GROUP ВЫПОЛНИТЬ
# ЗАПРОС НАЗВАНИЯ PRODUCT_TYPE ИЗ БД(ИЗ ТАБЛИЦЫ PRODUCT_GROUP) И ПО ПОЛУЧЕННОМУ
# PRODUCT_TYPE ЗАПРОСИТЬ АРТИКУЛЫ БАЗОВЫХ КАРТОЧКИ (ДЛЯ ОДЕЖДЫ МОЖЕТ БЫТЬ НЕСКОЛЬКО 
# Т.К. НЕСКОЛЬКО РАЗМЕРОВ) ИЗ ТАБЛИЦЫ OZON_BASE_CARD. ДАЛЕЕ ЗАПРОСИТЬ ВСЕ ЭТИ БАЗОВЫЕ
# КАРТОЧКИ С ОЗОНА 
